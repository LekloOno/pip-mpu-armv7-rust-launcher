...
pub fn m_create_partition(
    ...
    pip_rust_mpu::set_vidt(&pd_block_id, Some(vidt_addr as *const u32)).unwrap();
...
pub fn m_map_partition(partition_full_infos: CreateReturn) {
    pip_rust_mpu::map_mpu(
        ...
        Some(&partition_full_infos.partition.stack_vidt_block_id),
        0,
    )
    .unwrap();
    pip_rust_mpu::map_mpu(
        &partition_full_infos.parent_infos.pd_block_id,
        Some(&partition_full_infos.partition.ctx_itf_block_id),
        1,
    )
    .unwrap();
    pip_rust_mpu::map_mpu(
        &partition_full_infos.parent_infos.pd_block_id,
        Some(&partition_full_infos.partition.rom_block_id),
        2,
    )
    .unwrap();
    ...
    match partition_full_infos.partition.unused_ram_block_id {
        Some(x) => {
            pip_rust_mpu::map_mpu(&partition_full_infos.parent_infos.pd_block_id, Some(&x), 2)
                .unwrap()
        }
        _ => {}
    }
